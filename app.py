...
with st.sidebar:
    st.header("Menu")
    host, port = conn_info()
    st.caption(f"📡 Conectando a: `{host}:{port}`")
...
if menu == "Início":
    ...
    st.write(
        "Aplicação em Streamlit conectada a Postgres (Supabase). "
        "Use o menu lateral para cadastrar lançamentos e visualizar a DRE por competência."
    )
    st.info("Dica: clique em **Testar conexão** para verificar a URL e porta ativas; depois **Inicializar Banco de Dados**.")
